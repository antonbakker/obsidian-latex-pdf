...
%   * whitespace
%   * top thin line (via tcolorbox border)
%   * coloured icon + bold title
%   * thin line directly under the title
%   * callout text
%   * bottom thin line (via tcolorbox border)
%   * whitespace
% - Without a title (#1 empty):
%   * whitespace
%   * top thin line
%   * coloured icon in the left margin, text inside the normal text block
%   * bottom thin line
%   * whitespace
\newenvironment{callout-generic}[3][]{%
  ...
  \ifx\callout@title\@empty
    % No explicit title: icon slightly into the left margin, with text
    % remaining inside the normal text width.
    \hspace*{-1.2em}{#3}\hspace{0.75em}\begingroup\parbox[t]{\dimexpr\linewidth-1.95em\relax}%
  \else
    % Title present: coloured icon + bold title, then a thin rule before text.
    {\textcolor{#2}{#3\hspace{0.5em}\textbf{#1}}}\par\vspace{0.4ex}\hrule\vspace{0.75ex}%
  \fi
}{%
  \ifx\callout@title\@empty
    \endgroup
