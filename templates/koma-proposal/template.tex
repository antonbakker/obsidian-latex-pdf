...
Client: & $if(client)$$client$$else$\textit{(set client in frontmatter)}$endif$ \\
Project: & $if(project)$$project$$else$\textit{(set project in frontmatter)}$endif$ \\
Proposal ID: & $if(proposal_id)$$proposal_id$$else$\textit{(optional proposal_id)}$endif$ \\
